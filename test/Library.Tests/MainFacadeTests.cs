...
public class MainFacadeTests
{
    ...
    public void RegisterCallRegistersACall()
    {
        ...
        Client client =
            mainFacade.CreateClient("Juan", "Perez", "juanperez@gmail.com", "099888222", seller.Id.ToString());
    ...
    public void RegisterMeetingRegistersAMeeting()
    {
        ...
        Client client =
            mainFacade.CreateClient("Juan", "Perez", "juanperez@gmail.com", "099888222", seller.Id.ToString());
        mainFacade.RegisterMeeting("Expulsion de juan", "Rechazada", "Edificio de la empresa", "Done",
            client.Id.ToString(), "10/12/2025");
        Assert.That(client.Interactions.Count, Is.EqualTo(1));
    }
    ...
    [Test]
    public void GetClientHigherThanSell_ReturnsListWithClient1()
    {
        // Arrange
        string amount = "100";
        Seller seller = AdminFacade.Instance.CreateSeller("kiki");
        Client client1 =
            SellerFacade.Instance.CreateClient("Antonie", "Griezmann", "Griezmann7@gmail.com", "123456789", "0");
        AdminFacade.Instance.CreateOpportunity("Mate", "25", "Close", "0");
        AdminFacade.Instance.CreateOpportunity("Moto", "750", "Close", "0");
        // Act
        List<Client> result = AdminFacade.Instance.GetClientsHigherThanSell(amount);
        // Assert
        Assert.That(result, Does.Contain(client1));
    }

    [Test]
    public void GetClientHigherThanSell_ReturnsEmptyList_WhenStateIsNotClose()
    {
        // Arrange
        string amount = "5";
        Seller seller = AdminFacade.Instance.CreateSeller("kiki");
        Client client1 =
            SellerFacade.Instance.CreateClient("Antonie", "Griezmann", "Griezmann7@gmail.com", "123456789", "0");
        AdminFacade.Instance.CreateOpportunity("Mate", "25", "Open", "0");
        AdminFacade.Instance.CreateOpportunity("Moto", "750", "Canceled", "0");
        // Act
        List<Client> result = AdminFacade.Instance.GetClientsHigherThanSell(amount);
        // Assert
    ...
    [Test]
    public void GetClientHigherThanSell_ReturnsEmptyList_WhenAmountIsHigher()
    {
        // Arrange
        string amount = "1000";
        Seller seller = AdminFacade.Instance.CreateSeller("kiki");
